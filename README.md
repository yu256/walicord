...
1. **経費を記録する**: Discordでメンションを使って支払いを記録します
   ```
   @太郎 が @花子 に 1000円 貸した
   @Taro lent 1000 to @Hanako
   ```

2. **決済プランを確認する**: `!evaluate` コマンドで最適な決済方法を計算
   ```
   !evaluate
   ```
...
#### Discordメンションで指定
通常の Discord メンション機能を使ってユーザーを指定できます。メッセージを入力するときに `@` を入力してユーザー名を選択すると、自動的に `<@123456789>` の形式になります。
...
```
@太郎 が @花子 に 1000円 貸した
@花子 が @太郎 から 1000円 借りた
```

#### 英語での記録
```
@Taro lent 1000 to @Hanako
@Hanako borrowed 1000 from @Taro
```
...
- **`!evaluate`**: 現在の残高と最適な決済プランを計算して表示
...
複数のメンバーを同時に指定する場合は、スペースで区切ります：

```
@太郎 lent 3000 to @花子 @次郎 @三郎
→ 3人で均等に分割（各1000円ずつ）
```
...
**注意**: グループ名とメンションを組み合わせる場合は、明示的に `∪` を使用してください：
```
チーム ∪ @太郎
```
